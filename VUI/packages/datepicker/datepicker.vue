      ...
      <transition name="fade-up" mode="out-in">
        <div class="x-picker" v-show="isActive">
          <div class="x-picker_header x-clearfix">
            <span
              class="x-icon-chevrons-left x-btn-prve"
              @click="changePrevYear"
            >&lt;&lt;</span>
            <span
              class="x-icon-chevron-left x-btn-prve"
              @click="changePrevMonth"
            >〈 </span>
            <span>{{ nowTime.year }} 年 {{ nowTime.month + 1 }} 月</span>
            <span
              class="x-icon-chevrons-right x-btn-next"
              @click="changeNextYear"
            >&gt;&gt;</span>
            <span
              class="x-icon-chevron-right x-btn-next"
              @click="changeNextMonth"
            > 〉</span>
          </div>
          <ul class="x-picker_week x-clearfix">
            <li v-for="(week, index) in 7" :key="week">
              {{ backWeek(index) }}
            </li>
          </ul>
...
@import url(../../theme/transition.less);
